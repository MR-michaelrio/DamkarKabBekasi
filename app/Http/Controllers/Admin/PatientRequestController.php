...
class PatientRequestController extends Controller
{
    public function exportPdf(PatientRequest $patientRequest)
    {
        ...
        $pdf = Pdf::loadView('admin.reports.kebakaran_pdf', [
            ...
            'community_leader_name'  => $patientRequest->community_leader_name ?? '-',
            'community_leader_phone' => $patientRequest->community_leader_phone ?? '-',
            'area_size'       => $patientRequest->affected_area ?? '-',
            'building_type'   => $patientRequest->building_type ?? '-',
            'owner_name'      => $patientRequest->owner_name ?? '-',
            'owner_age'       => $patientRequest->owner_age ?? '-',
            'owner_phone'     => $patientRequest->owner_phone ?? '-',
            'owner_occupation'=> $patientRequest->owner_profession ?? '-',
            'fire_origin'     => $patientRequest->fire_cause ?? '-',
            ...
            'scba_usage'      => $patientRequest->scba_usage ?? '0',
            'apar_usage'      => $patientRequest->apar_usage ?? '0',
            'injured'         => $patientRequest->injured_count ?? '0',
            'fatalities'      => $patientRequest->fatalities_count ?? '0',
            'displaced'       => $patientRequest->displaced_count ?? '0',
            ...
            'ba_kronologi'       => $patientRequest->event_description ?? '-',
            'ba_lokasi_kebakaran'=> $lokasiLengkap ?: '-',
            'ba_jenis_bangunan'  => $patientRequest->building_type ?? '-',
            'ba_penyebab'        => $patientRequest->fire_cause ?? '-',
            'ba_luas_area'       => $patientRequest->affected_area ?? '-',
            'ba_nama_pemilik'    => $patientRequest->owner_name ?? '-',
            'ba_umur_pemilik'    => $patientRequest->owner_age ?? '-',
            'ba_pekerjaan_pemilik' => $patientRequest->owner_profession ?? '-',
            ...
            'ba_telp_rt_rw'      => $patientRequest->community_leader_phone ?? '-',
            ...
            'ba_scba_usage'      => $patientRequest->scba_usage ?? '0',
            'ba_apar_usage'      => $patientRequest->apar_usage ?? '0',
            'ba_korban_luka'     => $patientRequest->injured_count ?? '0',
            'ba_korban_jiwa'     => $patientRequest->fatalities_count ?? '0',
            'ba_korban_terdampak'=> $patientRequest->displaced_count ?? '0',
    ...
    {
        // Extract and format time fields to H:i format
        ...
        if ($request->filled('time_finished') && strpos($request->time_finished, ':') !== false) {
            $parts = explode(':', $request->time_finished);
            $request->merge(['time_finished' => $parts[0] . ':' . $parts[1]]);
        }
