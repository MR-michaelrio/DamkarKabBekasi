...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">
            🚒 Detail Laporan #<?php echo e($dispatch->nomor ?? $dispatch->id); ?>

        </h1>
        <div class="flex items-center gap-4">
            <a href="<?php echo e(route('admin.dispatches.export-single.pdf', $dispatch->id)); ?>" 
            ...
            <a href="<?php echo e(route('admin.dispatches.index')); ?>" class="text-gray-600 hover:text-gray-800 font-bold">
                ← Kembali
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white shadow rounded-xl p-6 border border-gray-100 lg:col-span-1 space-y-6">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-gray-900 mt-1"><?php echo e($dispatch->patient_name); ?></p>
                <span class="text-xs font-bold px-2 py-1 rounded 
                    <?php if(in_array($dispatch->patient_condition, ['emergency', 'kebakaran'])): ?> bg-red-100 text-red-700
                    <?php elseif($dispatch->patient_condition === 'rescue'): ?> bg-blue-100 text-blue-700
                    <?php else: ?> bg-gray-100 text-gray-700 <?php endif; ?>">
                    <?php echo e(strtoupper(str_replace('_', ' ', $dispatch->patient_condition))); ?>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-md font-bold text-gray-900 mt-1"><?php echo e($dispatch->nomor ?? '-'); ?></p>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-500 uppercase tracking-wider">No HP</label>
                    <p class="text-md font-bold text-red-600 mt-1"><?php echo e($dispatch->patient_phone ?? '-'); ?></p>
                </div>
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-500 uppercase tracking-wider">Unit Damkar</label>
                <p class="text-md text-gray-900 mt-1">👨‍✈️ <?php echo e($dispatch->driver?->name ?? '-'); ?></p>
                <p class="text-md text-gray-900 mt-1">🚒 <?php echo e($dispatch->ambulance?->plate_number ?? '-'); ?></p>
            </div>
            ...
            <div>
                ...
                <p class="text-md font-bold text-gray-900 mt-1">
                    <?php echo e($dispatch->request_date?->format('d F Y') ?? '-'); ?>

                </p>
                <p class="text-sm text-gray-600 border px-2 mt-1 rounded inline-block bg-gray-50">
                    Jam Kejadian: <?php echo e($dispatch->pickup_time ?? '-'); ?>
            ...
            <div>
                ...
                <p class="text-sm mt-1 bg-red-50 p-2 rounded font-bold border border-red-100"><?php echo e($dispatch->pickup_address); ?></p>
                
                <div class="grid grid-cols-2 gap-x-4 gap-y-2 mt-3 text-xs">
                    <div>
                        <span class="text-gray-400 font-bold uppercase block">Blok</span>
                        <span class="text-gray-700 font-bold"><?php echo e($dispatch->blok ?? '-'); ?></span>
                    </div>
                    <div>
                        <span class="text-gray-400 font-bold uppercase block">RT / RW</span>
                        <span class="text-gray-700 font-bold"><?php echo e($dispatch->rt ?? '-'); ?> / <?php echo e($dispatch->rw ?? '-'); ?></span>
                    </div>
                    <div>
                        <span class="text-gray-400 font-bold uppercase block">Kelurahan</span>
                        <span class="text-gray-700 font-bold"><?php echo e($dispatch->kelurahan ?? '-'); ?></span>
                    </div>
                    <div>
                        <span class="text-gray-400 font-bold uppercase block">Kecamatan</span>
                        <span class="text-gray-700 font-bold"><?php echo e($dispatch->kecamatan ?? '-'); ?></span>
                    </div>
                </div>
            </div>

            <?php if($dispatch->destination && $dispatch->destination !== '-'): ?>
            <div>
                <label class="block text-sm font-bold text-gray-500 uppercase tracking-wider">Tujuan (Opsional)</label>
                <p class="text-sm mt-1 bg-gray-50 p-2 rounded"><?php echo e($dispatch->destination); ?></p>
            </div>
            <?php endif; ?>
            ...
            <div>
                ...
                <div class="mt-1">
                    <span class="px-3 py-1 text-sm font-bold rounded shadow-sm
                        <?php if($dispatch->status === 'completed'): ?> bg-green-100 text-green-700
                        <?php elseif($dispatch->status === 'assigned'): ?> bg-blue-100 text-blue-700
                        <?php else: ?> bg-yellow-100 text-yellow-700 <?php endif; ?>">
                        <?php echo e(strtoupper(str_replace('_',' ', $dispatch->status))); ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const dispatchId = <?php echo e($dispatch->id); ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/Dev/damkar-dispatch/resources/views/admin/dispatches/show.blade.php ENDPATH**/ ?>